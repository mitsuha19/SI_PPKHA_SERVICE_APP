    ...
    <div class="main-content d-flex flex-column align-items-start gap-3">
        ...
        <div class="card-lowongan d-flex flex-row align-items-center gap-5">
            <img style="height: 92px; width: auto;"
                src="{{ !empty($lowongan->perusahaan->logo) ? config('services.main_api.url') . '/storage/' . ltrim($lowongan->perusahaan->logo, '/') : asset('public/assets/images/image.png') }}"
                alt="Logo Perusahaan">
        ...
        <div class="card-lowongan-about d-flex flex-row align-items-center gap-5">
            <div class="montserrat-medium mb-0 w-100">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    @php
                        $keahlian =
                            !empty($lowongan->keahlian) && is_string($lowongan->keahlian)
                                ? explode(',', $lowongan->keahlian)
                                : (is_array($lowongan->keahlian)
                                    ? $lowongan->keahlian
                                    : []);
                    @endphp
