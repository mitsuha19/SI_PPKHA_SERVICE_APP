    ...
    <div class="main-content d-flex flex-column align-items-start gap-3">
        <div class="card-perusahaan d-flex flex-row align-items-center gap-5">
            <img style="height: 92px; width: auto;"
                src="{{ $perusahaan->logo ? config('services.main_api.url') . '/api/perusahaan/' . $perusahaan->id . '/logo' : asset('assets/images/default-logo.png') }}"
                onerror="this.onerror=null; this.src='{{ asset('assets/images/default-logo.png') }}'"
                onload="console.log('Company logo loaded for ID {{ $perusahaan->id }}: ', this.src)">
                    ...
                    <div class="mb-2">
                        <div class="d-flex flex-row mb-3">
                            <img style="width: 100px;"
                                src="{{ $perusahaan->logo ? config('services.main_api.url') . '/api/perusahaan/' . $perusahaan->id . '/logo' : asset('assets/images/default-logo.png') }}"
                                onerror="this.onerror=null; this.src='{{ asset('assets/images/default-logo.png') }}'"
                                onload="console.log('Job logo loaded for company ID {{ $perusahaan->id }}: ', this.src)">
                            <div class="d-flex flex-column">
                                <h3 class="mb-0 ps-3">{{ $job->judulLowongan }}</h3>
                                <ul class="roboto-light text-black mb-1" style="font-size: 15px">
                                    <li>{!! nl2br(e(Str::limit($job->deskripsiLowongan, 50, '...'))) !!}</li>
                                </ul>
                            </div>
                        </div>
                        <div class="d-flex flex-row gap-2 flex-wrap">
                            <div class="pills">{{ strtoupper($perusahaan->lokasiPerusahaan) }}</div>
